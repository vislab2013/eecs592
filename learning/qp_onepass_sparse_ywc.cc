  ...
  for (int cnt = 0; cnt < n; cnt++) {
    ...
    if (Ci >= C && G > eps && A[i] < C && idI[j]-1 != i && idI[j] > 0) {
      ...
      if (Gp > eps || Gp < -eps) {
        ...
        // Update qp.w and qp.noneg: w = w + da*(x-x2)
        add(W, x, da);
        add(W,x2,-da);
        for (int d = 0; d < p; d++) {
    ...
    else if (PG > eps || PG < -eps) {
      // % Update qp.a(i)
      double da = MIN( MAX( G / D[i], -A[i] ), C - Ci );
      A[i] = A[i] + da;
